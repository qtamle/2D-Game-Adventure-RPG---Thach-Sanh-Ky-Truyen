...
public class SpikeRock : MonoBehaviour
{
    ...
    private IEnumerator MoveSpike(Vector2 from, Vector2 to, float speed)
    {
        ...
        {
            Vector2 newPosition = Vector2.Lerp(from, to, elapsedTime / duration);

            float offsetX = Random.Range(-1f, 1f) * shakeMagnitude;
            float offsetY = Random.Range(-1f, 1f) * shakeMagnitude;

            transform.position = new Vector3(newPosition.x + offsetX, newPosition.y + offsetY, transform.position.z);
